...
|---|---|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
| **cycle.access**<br>委員可見/可進入此週期(入口) | 中心 | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| | 機關 | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| | **委員** | | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
...
| | **機關** | | ✓ | | | | | |
| | 委員 | | | | | | | |
| **audit.score**<br>委員實地稽核評分與發現(頁/磚) | 中心 | | | | | | | |
| | 機關 | | | | | | | |
| | **委員** | | | | ✓ | ✓ | ✓ | ✓ |
| **deficiencies.view**<br>缺失與矯正管考(頁/磚) | 中心 | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| | **機關** | | | | | | ✓ | ✓ |
| | **委員** | | | | | ✓ | ✓ | ✓ |
...
## 重點規則(白話)
- **委員看得到週期的分界 = 離開開立中(PREPARATION 起)**:開立中(DRAFT)中心仍在頻繁調整委員名單,委員尚不可見/不可進入該週期;一進入資料準備中即可見。中心指派/抽換委員不受此閘限制(任何階段都可調整)。
...
- **委員實地稽核評分(audit.score)**:委員於「實地稽核(ONSITE)」階段才開放評分與發現(資料齊備階段僅供熟悉受稽機關背景,尚不評分);中心改看彙整報告。
- **缺失與矯正管考(deficiencies.view)**:中心全程;**機關待「矯正執行中(REMEDIATION)」**後才開放填報矯正(缺失發布中為中心發布期,機關尚不填);**委員待「缺失發布中(REPORT_ISSUED)」**後可審。
- **用印掃描檔**:只在矯正執行中之後出現,且只給機關(上傳)+ 中心(確認/檢視);委員不參與。
- **彙整報告**:中心專用;委員列印自己的附件17、機關不涉入。

> 註:`audit.score` / `deficiencies.view` 為委員模組「開放時機」閘,於**頁面(redirect)+ 入口磚**對委員強制(委員實際進入路徑);評分/發現 API 另以「未結案 + 未鎖定」把關(委員為受指派之信任角色,階段時機由頁面層把關,非租戶邊界)。
