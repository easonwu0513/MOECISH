...
export async function assertEvidenceAccess(targetType: string, targetId: string) {
  ...
  // 事前場次調查之檔案(cv/切結書/公版範本/舊版經歷說明書參考)不走共用 /api/evidences(其全域擋委員/觀察員上傳,
  // 而受調委員/觀察員須能上傳/下載自己的文件),改由 /api/pre-survey/** 專用路由自管授權(中心∨本人;範本公開下載)。
  // 此處明確拒絕(含 SURVEY_CV_PRIOR),防生成式共用路由繞過場次調查授權的 self∨center 閘。
  if (
    targetType === 'SURVEY_CV' ||
    targetType === 'SURVEY_NDA' ||
