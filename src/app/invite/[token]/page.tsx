    ...
    <AuthLayout title="MOECISH" subtitle="資通安全稽核管考平台">
          {!inv || status === 'revoked' || status === 'used' || status === 'expired' ? (
            <div className="text-center">
              ...
              <h2 className="text-title-lg text-ink-900">
            ...
            <>
              <div className="mb-5">
                <h2 className="text-title-lg text-ink-900">歡迎，{inv.name}</h2>
                <p className="mt-1 text-body-sm text-ink-500">
                  {inv.organization
                    ? <>您已被邀請加入 <span className="font-medium text-ink-900">{inv.organization.name}</span></>
                    : '您已被邀請加入 MOECISH'}
                </p>
